...
async def test_intent_analyzer_returns_dict():
    ...
    with patch(
        ...
        result = await intent_analyzer(_make_state("ô tô vượt đèn đỏ phạt bao nhiêu?"))
...
async def test_intent_analyzer_needs_clarification():
    ...
    with patch(
        ...
        result = await intent_analyzer(_make_state("vượt đèn đỏ phạt bao nhiêu?"))
...
async def test_intent_analyzer_out_of_scope_skips_clarification():
    ...
    with patch(
        ...
        result = await intent_analyzer(_make_state("doi bang lai xe can giay to gi"))
...
async def test_intent_analyzer_history_fills_missing_entities_without_overriding_current_query():
    ...
    with patch(
        ...
        state["messages"] = [
            ...
            {"role": "assistant", "content": "Muc phat ..."},
        ]
...
async def test_intent_analyzer_current_query_can_override_previous_violation():
    ...
    with patch(
        ...
        state = _make_state("con di sai lan thi sao?")
        state["messages"] = [
            {"role": "user", "content": "o to vuot den do phat bao nhieu"},
            {"role": "assistant", "content": "Muc phat ..."},
        ]
...
@pytest.mark.parametrize(
    ...
    [
        ("xe con vuot den do phat the nao", "ô tô", "vượt đèn đỏ"),
        ("xe gan may vuot den do phat the nao", "xe máy", "vượt đèn đỏ"),
        ("xe mo to khong doi mu bao hiem phat bao nhieu", "xe máy", "không đội mũ bảo hiểm"),
    ],
)
